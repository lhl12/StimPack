...
%   burst_limits: the start (and, optionally, end) indices of each 
%       stimulation burst, as generated by getStimIndices() OR
%       a trials x 1 array containing only burst start times
...
%   ASSUMPTIONS:
    ...
    addRequired(p, 'burst_limits', validLims);
    ...
    if adjust && isnan(adj_by) % if adjustment is specified but no value is given
        ...
        elseif ~isempty(diffPks)
            ...
            artStarts = [pklocs(1); pklocs(find(diffPks > min(diffPks) + range(diffPks)/2) + 1)];
            ...
            adj_by = ceil(median(min(abs(round(artStarts'-dataStarts))))) - 1;
